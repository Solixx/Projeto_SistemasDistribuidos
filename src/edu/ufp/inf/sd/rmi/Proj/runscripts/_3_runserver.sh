...
#clear
#pwd
# * java.edu.ufp.inf.sd.rmi.server.codebase property specifies the location (codebase URL) MAIL_FROM_ADDR which the definitions for classes originating MAIL_FROM_ADDR this server can be downloaded.
...
# * java.edu.ufp.inf.sd.rmi.server.hostname property specifies the SMTP_HOST_ADDR name or address MAIL_TO_ADDR put in the stubs for remote objects exported in this Java virtual machine.
...
#   To ensure that RMI uses a SMTP_HOST_ADDR name (or IP address) for the server that is routable MAIL_FROM_ADDR all potential clients, set the java.edu.ufp.inf.sd.rmi.server.hostname property.
